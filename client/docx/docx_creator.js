...
const mapRequestToDocxRows = (requestData) =>
  (Array.isArray(requestData?.items) ? requestData.items : []).map((item, index) => ({
...
const createBorders = () => ({
  top: { style: BorderStyle.SINGLE, size: DOCX_CONFIG.table.borderSize, color: DOCX_CONFIG.table.borderColor },
  bottom: { style: BorderStyle.SINGLE, size: DOCX_CONFIG.table.borderSize, color: DOCX_CONFIG.table.borderColor },
  left: { style: BorderStyle.SINGLE, size: DOCX_CONFIG.table.borderSize, color: DOCX_CONFIG.table.borderColor },
  right: { style: BorderStyle.SINGLE, size: DOCX_CONFIG.table.borderSize, color: DOCX_CONFIG.table.borderColor },
});
  ...
  new TableCell({
    ...
    children: [
      new Paragraph({
        ...
        spacing: { before: 0, after: 0, line: DOCX_CONFIG.table.lineSpacing },
        children: [
          new TextRun({
            text: text ?? "",
  ...
  return selectedSigners.map(
    (signer, index) =>
      new Paragraph({
        ...
        children: [new TextRun({ text: `${signer} ${DOCX_CONFIG.signers.lineText}` })],
      })
  );
};
...
const createTopHeaderParagraphs = () => [
  new Paragraph({
    ...
    spacing: { after: DOCX_CONFIG.header.afterLogoSpacing },
    children: [
      new ImageRun({
        data: base64Str,
        transformation: {
  ...
  new Paragraph({
    spacing: { after: 0 },
    tabStops: [{ type: TabStopType.RIGHT, position: DOCX_CONFIG.header.rightTabPosition }],
    children: [
      new TextRun({ text: "_____________" }),
      new TextRun({ text: "\tВ РАБОТУ _____________/___________/", bold: true, italics: true }),
    ],
  }),
  new Paragraph({
    spacing: { after: 40 },
    tabStops: [{ type: TabStopType.RIGHT, position: DOCX_CONFIG.header.rightTabPosition }],
    children: [
      new TextRun({ text: "(дата)", size: DOCX_CONFIG.font.smallSize }),
      new TextRun({ text: "\t(кому)", size: DOCX_CONFIG.font.smallSize }),
    ],
  }),
  new Paragraph({ spacing: { after: 0 }, children: [new TextRun({ text: "_____________________" })] }),
  new Paragraph({
    spacing: { after: DOCX_CONFIG.header.topLabelsAfterSpacing },
    children: [new TextRun({ text: "(стр. подразделение)", size: DOCX_CONFIG.font.smallSize })],
  }),
  new Paragraph(""),
  new Paragraph({
    indent: { left: DOCX_CONFIG.header.receiverIndentLeft },
    children: [
      new TextRun({ break: 1, text: "Генеральному директору" }),
      new TextRun({ break: 1, text: "ООО «Уралшина»" }),
      new TextRun({ break: 1, text: "Мустаеву М.Г." }),
    ],
  }),
  new Paragraph(""),
  new Paragraph(""),
...
const createRequestTable = (rows) =>
  new Table({
    width: { size: DOCX_CONFIG.table.width, type: WidthType.DXA },
    layout: "fixed",
    ...
    rows: [
      ...
      ...rows.map(
        (row) =>
          new TableRow({
            children: [
              createCell(row.number, { width: DOCX_CONFIG.table.columnWidths[0] }),
              createCell(row.title, {
                alignment: AlignmentType.LEFT,
                width: DOCX_CONFIG.table.columnWidths[1],
              }),
              createCell(row.units, { width: DOCX_CONFIG.table.columnWidths[2] }),
...
export const docxCreator = (requestData, selectedSigners = []) => {
    ...
    sections: [
      {
        ...
        children: [
          ...
          new Paragraph({
            alignment: AlignmentType.CENTER,
            spacing: { after: DOCX_CONFIG.title.afterSpacing },
            children: [
              new TextRun({
          ...
          new Paragraph({
            spacing: { after: DOCX_CONFIG.purpose.afterSpacing },
            children: [
              new TextRun({ text: DOCX_CONFIG.purpose.label, bold: true }),
              new TextRun({ text: purchasePurpose }),
            ],
          }),
  ...
  Packer.toBlob(doc).then((blob) => {
    saveAs(blob, requestData?.id ? `zayavka-${requestData.id}.docx` : "zayavka.docx");
  });
};